...
static BLEUUID charUUID_Tx(CHARACTERISTIC_UUID_RX);
static BLEUUID charUUID_Rx(CHARACTERISTIC_UUID_TX);

static boolean doConnect = false;
static boolean connected = false;
static boolean doScan = false;
static BLERemoteCharacteristic *pRemoteCharacteristic_Tx, *pRemoteCharacteristic_Rx;
...
bool connectToServer() {
    ...
    if( pRemoteCharacteristic_Tx == nullptr) {
      Serial.print("Filed to find tx UUID: ");
...
void setup() {
  ...
  BLEDevice::init("ESP slave board 1");
...
void loop() {
  ...
  if (connected) {
    ...
    if(Serial.available()){
      ...
      pRemoteCharacteristic_Tx->writeValue(txValue.c_str(), txValue.length());
      delay(10);
    } 
  }
  else if(doScan){
