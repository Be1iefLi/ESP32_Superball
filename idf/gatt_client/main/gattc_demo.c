...
#define REMOTE_NOTIFY_CHAR_UUID    0xFF03
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        if(p_data->notify.value[0] == 0x0a){
            next_status = p_data -> notify.value[1];
        }
        break;
...
{
    int16_t Position, Old_Position;
    portBASE_TYPE res;
    uint8_t data[10];
    data[0] = 0x0a;
    while (1) {
        if(next_status != 0x01 && (status&0xf0) == 0x00){
            status = next_status;
            switch (next_status)
            {
            case 0x11:
            ...
            case 0x10:
                ...
                ESP_LOGI(MOTOR_TASK_TAG, "Write %d bytes data:", 2);
                esp_log_buffer_hex(MOTOR_TASK_TAG, data, 2);
